...
redis-server &
dd if=/dev/random of=/etc/munge/munge.key count=1
chown munge /etc/munge/munge.key
runuser -u munge -- /usr/sbin/munged  --force -F &

echo "Starting Shifter Services"
/usr/bin/gunicorn -b 0.0.0.0:5000 --backlog 2048 --access-logfile=/var/log/shifter_imagegw/access.log --log-file=/var/log/shifter_imagegw/error.log shifter_imagegw.api:app &
celery -A shifter_imagegw.imageworker worker -Q mycluster -n 1 --loglevel=debug --logfile=/var/log/shifter_imagegw/mycluster.log &
